			...
			if (!is_array($this->_sValidator))
			{
				...
				if( strpos( $this->_sValidator, '|' ) > 0 )
				{
					...
					foreach( $aValidator AS $val )
					{
						...
						{
							$value = $this->getValue();
							$error = call_user_func( $val, is_string($value) ? trim( $value) : $value );
						}
						else
						{
							$v = new Validator();
...
//					$error = call_user_func(array(&$this->_sValidator[0], $this->_sValidator[1]), trim($this->getValue()) );
					...
					if ( count( $this->_sValidator ) > 2 )
                    {
                        $sCall = "return call_user_func(array(&"."$"."this->_sValidator[0], "."$"."this->_sValidator[1]), trim( "."$"."this->getValue() )";
                        for ( $i = 2; $i < count( $this->_sValidator ); $i++ )
                        {
                            $sCall .= ", "."$"."this->_sValidator[".$i."]";
                        }
                        $error = eval( $sCall . " );");
